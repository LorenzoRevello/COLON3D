...
def load_poses_from_pos_rot_files(position_file, rotation_file, use_quaternion=True):
    ...
    locations = np.array(locations) * 10.0
...
def remove_single_triangles(mesh):
    ...
    remove_mask = np.array([np.asarray(cluster_n_triangles)[c] == 1 for c in np.asarray(triangle_clusters)], dtype=bool)
...
def load_gt_mesh_from_stl(stl_path):
    print(f"\nLoading GT mesh from: {stl_path}")
    if not Path(stl_path).exists():
        raise FileNotFoundError(f"STL file not found: {stl_path}")
    mesh_gt = o3d.io.read_triangle_mesh(str(stl_path))
    if not mesh_gt.has_vertex_normals(): mesh_gt.compute_vertex_normals()
    print(f"✅ Loaded GT mesh: {len(mesh_gt.vertices):,} vertices, {len(mesh_gt.triangles):,} triangles")
    return mesh_gt


def poisson_closing_pipeline(mesh_original, trajectory_xyz, output_path, poisson_depth=11, seed_points=90000, missing_min_triangles=5, missing_min_area=1.0, missing_density_threshold=10):
    print("\n=== POISSON MESH CLOSING ===")
    ...
    mesh_poisson, _ = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
        mesh_original.sample_points_uniformly(number_of_points=seed_points), depth=poisson_depth
    )
    ...
    if missing_mesh and input("Save missing regions mesh? (y/n): ").strip().lower() in ["y", "yes"]:
...
if __name__ == "__main__":
    ...
    print("Choose reference mesh for missing regions extraction:")
    print("1. Poisson reconstruction (auto-generated)")
    print("2. Ground Truth mesh from STL file (manual)")
    
    if input("\nChoice (1 or 2): ").strip() == "2":
        gt_stl_path = str(Path("./dataset") / "colon_model_v3_STL.stl")
        try:
            mesh_gt = load_gt_mesh_from_stl(gt_stl_path)
            closed_mesh, missing_mesh, _, timing = gt_mesh_missing_regions_pipeline(
                geo, mesh_gt, trajectory_xyz, str(output_dir / "mesh_gt_reference.ply")
            )
        except Exception as e:
            print(f"❌ Error loading GT mesh: {e}")
            raise SystemExit(1)
    else:
        closed_mesh, missing_mesh, _, timing = poisson_closing_pipeline(
            geo, trajectory_xyz, str(output_dir / "mesh_closed_poisson.ply")
        )
